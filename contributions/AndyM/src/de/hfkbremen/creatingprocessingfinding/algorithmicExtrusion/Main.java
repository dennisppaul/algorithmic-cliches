...
import de.hfkbremen.creatingprocessingfinding.voronoidiagram.Qvoronoi;
...
public class Main
    ...
    public void setup() {
        ...
        for (int x = 0; x < vertexresult.width; x++) {
            for (int y = 0; y < vertexresult.height; y++) {
                if (vertexresult.get(x, y) != color(0)) {
                    vertices.add(new Vector3f(x, y));
                }
            }
        }
        ...
        TreeSet<Vector3f> qvoverts = new TreeSet<Vector3f>();
        int numverts = 0;
        ...
        qvovertsExt = new TreeSet<Vector3fExt>();
